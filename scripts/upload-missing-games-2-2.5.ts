...
import { missingGames_2_2_5 } from './create-missing-games-2-2.5';
import { missingGames_2_2_5_part2 } from './create-missing-games-2-2.5-part2';
import { missingGames_2_2_5_part3 } from './create-missing-games-2-2.5-part3';

config({ path: '.env.local' });

const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);

async function uploadMissingGames() {
  console.log('=== 上传2-2.5岁年龄段缺失游戏到数据库 ===\n');
  
  // 合并所有游戏
  const allMissingGames = [
    ...missingGames_2_2_5,
    ...missingGames_2_2_5_part2,
    ...missingGames_2_2_5_part3
  ];
  
  console.log(`总共准备上传 ${allMissingGames.length} 个游戏\n`);
  
  try {
    // 转换游戏数据格式，添加缺失字段
    const gamesToUpload = allMissingGames.map((game) => ({
      ...game,
      name: game.name,
    ...
    console.log(`2-2.5岁年龄段覆盖情况: ${coveredCombinations}/${totalCombinations} (${(coveredCombinations/totalCombinations*100).toFixed(1)}%)`);
    
    console.log('\n🎉 2-2.5岁年龄段缺失游戏补充完成！');
    console.log('📈 这些游戏将显著改善该年龄段的游戏覆盖率！');
...
// 运行上传
uploadMissingGames().catch(console.error);