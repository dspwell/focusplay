...
console.log('=== 20个旅途中场景游戏总览 ===');
console.log('');

// 按年龄段统计
const ageGroups = {
  '2-2.5': travelGames.filter(g => g.age_range === '2-2.5').length,
  '2.5-3': travelGames.filter(g => g.age_range === '2.5-3').length,
  '3-4': travelGames.filter(g => g.age_range === '3-4').length
};
...
// 按道具类型统计
const propsGroups = {
  'hands': travelGames.filter(g => g.props === 'hands').length,
  'paper': travelGames.filter(g => g.props === 'paper').length,
  'household': travelGames.filter(g => g.props === 'household').length,
  'books_pictures': travelGames.filter(g => g.props === 'books_pictures').length
};

console.log('\n道具类型分布：');
Object.entries(propsGroups).forEach(([prop, count]) => {
  console.log(`  ${prop}: ${count}个游戏`);
});

// 按能力发展统计
const focusGroups = {
  'cognition': travelGames.filter(g => g.focus === 'cognition').length,
  'language': travelGames.filter(g => g.focus === 'language').length,
  'fine': travelGames.filter(g => g.focus === 'fine').length,
  'gross': travelGames.filter(g => g.focus === 'gross').length,
  'social': travelGames.filter(g => g.focus === 'social').length
};

console.log('\n能力发展分布：');
Object.entries(focusGroups).forEach(([focus, count]) => {
  console.log(`  ${focus}: ${count}个游戏`);
});

console.log('\n游戏列表：');
travelGames.forEach((game, index) => {
  console.log(`${index + 1}. ${game.name} (${game.age_range}岁, ${game.props}, ${game.focus})`);
});
